...
class DbCheck extends BaseController
{
    public function index()
    {
        ...
        try {
            ...
            $out['indexes']['visits_idx'] = 'ERROR: ' . $e->getMessage();
        }

        foreach (['pet_id','mime','note','visit_id','filename'] as $col) {
            try {
                $row = $db->query("SHOW COLUMNS FROM documents WHERE Field=?", [$col])->getRowArray();
                $out['columns'][$col] = $row ? 'OK' : 'MISSING';
            } catch (\Throwable $e) {
                $out['columns'][$col] = 'ERROR: ' . $e->getMessage();
            }
        }

        foreach (['pets','visits','documents'] as $t) {
            try {
                $c = $db->table($t)->countAllResults();
                $out['counts'][$t] = $c;
            } catch (\Throwable $e) {
        ...
        <table class="table table-sm">
          ...
          <tbody>
            <tr><td>visits: idx_visits_pet_date_seq</td><td><?= esc($out['indexes']['visits_idx']) ?></td></tr>
          </tbody>
        </table>
        ...
        <table class="table table-sm">
          ...
          <tbody>
          <?php foreach ($out['columns'] as $k=>$v): ?>
        ...
        <table class="table table-sm">
          ...
          <tbody>
          <?php foreach ($out['counts'] as $k=>$v): ?>
            <tr><td><?= esc($k) ?></td><td><?= esc(strval($v)) ?></td></tr>
          <?php endforeach; ?>
          </tbody>
        </table>
