...
class Settings extends BaseController
{
    ...
    public function save(): ResponseInterface
    {
        ...
        $this->settings->put('auto_map_orphans', $auto);
        $this->settings->put('date_display_format', $format);
        $this->settings->put('visits_lite_public', $pub);
    ...
    // POST /admin/tools/settings/toggle  (legacy single-flag toggle kept)
    public function toggle(): ResponseInterface
    {
        $enable = $this->request->getPost('enable') === '1' ? '1' : '0';
        $this->settings->put('auto_map_orphans', $enable);
